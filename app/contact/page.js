  ...
  return (
    <main className="pt-28">
      <section className="section-pad">
        <Container>
          <SectionHeader eyebrow="Contact" title="Tell us what you want to build." text="Use the form for project details, or contact directly through WhatsApp and email for faster conversation." />
            ...
            <Reveal delay={0.1} className="space-y-5">
              <div className="rounded-[1.25rem] border border-line bg-coal p-6">
                ...
                <div className="mt-5 grid gap-3 text-muted">
                  <a href={`tel:${site.phone.replace(/\s/g, "")}`} className="flex min-h-11 items-center gap-3 hover:text-lime"><Phone size={18} /> {site.phone}</a>
                  <a href={`mailto:${site.email}`} className="flex min-h-11 items-center gap-3 hover:text-lime"><Mail size={18} /> {site.email}</a>
                  <a href={site.whatsapp} className="flex min-h-11 items-center gap-3 text-lime"><MessageCircle size={18} /> WhatsApp click-to-chat</a>
                  <p>{site.location}</p>
                </div>
      ...
      <section className="section-pad bg-[#080a08]">
        <Container>
          ...
          <Reveal className="mx-auto mt-10 max-w-3xl"><Faq /></Reveal>
        </Container>
      </section>
    </main>
  );
